...
if(!$edit && !$save && !$new) {
	...
	$pageTitle = "$filename - Applications"; 
}

$dirlist = $dir->read_dir(SITE_PATH."/apps/".$dirname, false, $blacklist_dir, $blacklist_file); /* allowing recursive mode is insecure and often process intensive to run, use at own risk */
...
$col = 0;
	...
	$max = $limit * $show; 
			...
			$list = "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"10\" border=\"0\">
						<tr>";
		for($i=0;$i < $count;$i++) {
			$filename = (!is_dir(SITE_PATH."/apps/".$dirlist[$i]) ? substr($dirlist[$i], 0, (strlen($dirlist[$i])-4) ) : $dirlist[$i]);
			...
			//if($fileInfo['id'] == "") $query->insert("pages", "id,title,description,permissions,owner", "'".$filename."','".$filename."','','".$auth->getLevel()."','".ID."'", $debug);
					if(($fileInfo['permissions'] <= $auth->getLevel() || $fileInfo['owner'] == ID) && (($cnt <= $max) && (($i <= $max) && ($i >= $min)) || (($i >= $min) && ($i <= $max)))) {
						...
						if(is_dir(SITE_PATH."/apps/".$dirlist[$i])) {
							$list .= "<a href=\"scripts?d=".$dirlist[$i]."\" title=\"".$dirlist[$i]." folder.\">
								<img 
							...
							$list .= "
									<small>
<a href=\"javascript:uiConfirm('scripts?p=".$filename."&".(isset($dirname) ? "d=".$dirname."&" : "")."del', '".$filename."');\">delete</a>
									</small>
									<br>
									<br>
									";
