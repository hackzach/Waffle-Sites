...
$blacklist_dir = array("styles", "main");
...
$dirlist = $dir->read_dir(SITE_PATH."/pages/home/i/".ID."/".(isset($dirname) ? "/".$dirname : NULL), false, $blacklist_dir, $blacklist_file);
...
$col = 0;
	...
	$max = $limit * $show; 
			...
			$list = "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"10\" border=\"0\">
						<tr>";
		for($i=0;$i < $count;$i++) {
					$filename = (!is_dir(SITE_PATH."/pages/home/i/".ID."/".(isset($dirname) ? $dirname."/" : NULL).$dirlist[$i]) ? substr($dirlist[$i], 0, (strlen($dirlist[$i])-5) ) : $dirlist[$i]);
					...
					if(($fileInfo['permissions'] <= $auth->getLevel() || $fileInfo['owner'] == ID) && $filename != "authorize" && $filename != "graph" && (($cnt <= $max) && (($i <= $max) && ($i >= $min)) || (($i >= $min) && ($i <= $max)))) {
						...
						if(is_dir(SITE_PATH."/pages/home/i/".ID."/".(isset($dirname) ? $dirname."/" : NULL).$dirlist[$i])) {
							$list .= "<a href=\"content?d=".(isset($dirname) ? $dirname."/" : NULL).$dirlist[$i]."\" title=\"".$dirlist[$i]." folder.\">
								<img 
							...
							$list .= "	<small>
									<a href=\"javascript:uiConfirm('content?p=".
